...
do while (it<=i%nit)
   ...
   if (rank.eq.0)     write(*,*) "+++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++" 
   if (rank.eq.0)     write(*,*) "=============================================================================" 
   !if (rank.eq.0)     write(*,*) "===========Frequency Multiscale  FWI Iterations == ",it
   if (rank.eq.0)     write(*,*) "===========  Frequency Multiscale (==>",fr_Ind,"        <==) FWI " 
   !if (rank.eq.0)     write(*,*) "f11=",f11,"Hz,f22=",f22, "Hz,f33=",f33,"Hz,f44=",f44,"Hz" 
   ...
   do ipro=1,npro_me
      ...
      xs=c%xs
      xg=c%xg
      if (o%isCut)  call csg_cut(sh%seis,data_true,sh%nt,sh%ng, xs,xg, o%cut_distance)
  ...
  if (rank.eq.0) then
   ...
   alpha=pur*sqrt(sum(ss**2))/(0.01*maxval(m%refl)+sqrt(sum(m%refl**2)))
...
write(*,*)
if (o%isCut) write(*,*)"Cut Near-off Traces=",o%ncut
