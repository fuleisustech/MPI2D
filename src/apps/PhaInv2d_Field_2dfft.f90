...
real                 :: t1, t2,pur=0.01,f1,f2,f3,fff ! fff is the true step length ratio
...
do while (it<=i%nit)
   ...
   if ((rank.eq.0).and.(o%is_conv_sou.eq.1))     write(*,*) "!!!!!!!! Conv with sources !!!!!!!!!" 
   if ((rank.eq.0).and.(o%grad_flip))     write(*,*) "!!!!!!!! GRAD FLIP !!!!!!!!!" 
   if (rank.eq.0)     write(99,*) "!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!" 
   if (rank.eq.0)     write(99,*) "!!!!  Frequency Multiscale (==>",fr_Ind,"  <==) MPI2d !!!!!" 
   if (rank.eq.0)     write(99,*) "!" ,f11,f22,f33,f44," Hz !" 
   if (rank.eq.0)     write(99,*) "!   PEAK FREQ " ,o%fpk, " Hz !" 
   if (rank.eq.0)     write(99,*) "!!!!!!!!  Integrations Num =  ",N,"!!!!!!!!!!!!!!!!" 
   if ((rank.eq.0).and.(N.eq.-1))     write(99,*) "!!!!!!!!  I am traditional FWI Here !!!!!!!!!" 
   if ((rank.eq.0).and.(o%is_conv_sou.eq.1))     write(99,*) "!!!!!!!! Conv with sources !!!!!!!!!" 
   if ((rank.eq.0).and.(o%grad_flip))     write(99,*) "!!!!!!!! GRAD FLIP !!!!!!!!!" 
   ...
   !  call data_res(sh%seis,data_true,sh%nt,sh%ng,sh%dt,res_data,res_dt,N,f11,f22,f33,f44,o%use_amp) !use 2dfft
      call data_res(sh%seis,data_true,sh%nt,sh%ng,sh%dt,res_data,res_dt,N,f11,f22,f33,f44,mask_csg) ! mask the adjoint source
      ...
      !if (o%is_output_residual)  then
      !call filename(output, fn%csg_out, c%proid(ipro), "_res_2dfft.bin")
      !call write_binfile(output,sh%seis,sh%nt, sh%ng)
      !endif
...
enddo ! Inversion loop end 
   ...
   if (rank == 0) write(99,*) 'Runtime = ', t2-t1
contains
