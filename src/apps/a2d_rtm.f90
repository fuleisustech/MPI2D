...
call allocate_and_initial(image_temp,m%nz,m%nx)
if (o%isIllum) then
   call allocate_and_initial(image_illum_temp,m%nz,m%nx)
endif
do ipro=1,npro_me
   ...
   if (o%isIllum) then
      img%img=img%img/img%illum
      call allocate_and_initial(temp,m%nz,m%nx)
      call aperture_image_l2g(a%isAper,img%img,img%iz1,img%iz2,img%ix1,img%ix2,&
                              temp,m%nz,m%nx)
      image_illum_temp=image_illum_temp+temp
      call deallocate_and_free(temp)
   endif
...
call sf90_sum(image_temp,image,m%nz,m%nx)
!call MPI_REDUCE(image_temp,image,m%nz*m%nx,MPI_REAL,MPI_SUM,&
!        0,MPI_COMM_WORLD,ierr)
if (rank.eq.0) then
   call write_binfile(fn%img_file,image,m%nz,m%nx)
