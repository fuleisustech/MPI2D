module module_resort3d
use module_datatype
use module_array
use module_sf90_mpi
implicit none
real,    allocatable, private :: xs(:),ys(:),zs(:),xg(:),yg(:),zg(:),t(:)
integer, allocatable, private :: sid(:),gid(:),num_chan(:)

private convert_1d_to_2d, convert_2d_to_1d

contains

!=======================================================================
!subroutine csg2crg_resort(c1,c2,indx,sort_tol)
!type(coord3d), intent(in)  :: c1
!type(coord3d), intent(inout) :: c2
!integer,       intent(out) :: indx(:)
!real,          intent(in)  :: sort_tol
!integer itrace, ipro, ig
!c2%ntrace=c1%ntrace
!call allocate_and_initial(xs,ys,zs,xg,yg,zg,t,c1%ntrace)
!call allocate_and_initial(sid,gid,num_chan,c1%ntrace)
!call convert_2d_to_1d(c1)
!call resort_core(c1%ntrace,xg,yg,xs,ys,c2%npro,num_chan,indx,sort_tol)
...
!integer itrace, ipro, ig
!c2%ntrace=c1%ntrace
!call allocate_and_initial(xs,ys,zs,xg,yg,zg,t,mp,c1%ntrace)
!call allocate_and_initial(sid,gid,num_chan,c1%ntrace)
!call convert_2d_to_1d(c1)
!mp=(xs+xg)/2.0
...
!=======================================================================
!subroutine resort_core2(ntrace,rec1,rec2,rec3,rec4,npro,num_chan,indx_out,sort_tol)
!integer,   intent(in)  :: ntrace
!real,      intent(in)  :: rec1(:), rec2(:), rec3(:), rec4(:), sort_tol
!integer,   intent(out) :: npro, num_chan(:),indx_out(:)
!integer,   allocatable :: indx(:),indx1(:),indx2(:),indx3(:)
!real,      allocatable :: rec1_new(:),rec2_new(:), rec3_new(:), rec4_new(:)
!integer                :: itrace, itrace1, itrace2, ii
!call allocate_and_initial(indx,indx1,indx2,indx3,ntrace)
!call allocate_and_initial(rec1_new,rec2_new,ntrace)
!call indexx(ntrace,rec1,indx)
...
!   rec1_new(itrace)=rec1(indx(itrace))
!   rec2_new(itrace)=rec2(indx(itrace))   
!   rec3_new(itrace)=rec3(indx(itrace))
!   rec4_new(itrace)=rec4(indx(itrace))   
!enddo
!npro=1
!num_chan(npro)=1
!itrace1=1
...
!   if (abs(rec1_new(itrace)-rec1_new(itrace-1))>sort_tol) then
!      itrace2=itrace-1
!      ntrace_temp=itrace2-itrace1+1
!      call resort_core(ntrace_temp,rec2_new(itrace1:itrace2),&
!         rec3_new(itrace1:itrace2),
!      indx1(1:num_chan(npro))=indx(itrace1:itrace2)
!      call indexx(num_chan(npro),rec2_new(itrace1:itrace2),indx2(1:num_chan(npro)))
!      do ii=1,num_chan(npro)
!         indx3(ii)=indx1(indx2(ii))
...
do ipro=1,c1%npro
   do ig=1,c1%ng(ipro)
      ...
      ys(itrace)=c1%ys(ig,ipro)
      zs(itrace)=c1%zs(ig,ipro)
      xg(itrace)=c1%xg(ig,ipro)
      yg(itrace)=c1%yg(ig,ipro)
...
do ipro=1,c2%npro
   ...
   do ig=1,c2%ng(ipro)
      ...
      c2%ys(ig,ipro)=ys(indx(itrace))
      c2%zs(ig,ipro)=zs(indx(itrace))
      c2%xg(ig,ipro)=xg(indx(itrace))
      c2%yg(ig,ipro)=yg(indx(itrace))
