...
interface csg_filt   ! Lei Fu
   module procedure csg_filt_1d
   module procedure csg_filt_2d
end interface csg_filt

interface csg_conv_sou   ! Lei Fu
   module procedure csg_conv_sou_2d
!   module procedure csg_conv_sou_3d
end interface csg_conv_sou


interface data_conv   ! Lei Fu
   module procedure data_conv_1d
   module procedure data_conv_2d
end interface data_conv

interface ricker0 ! Lei Fu
   module procedure ricker0_1d
end interface ricker0
...
interface data_res   ! Lei Fu : data_res for wt
   ...
   module procedure data_res_phainv_2dfft
   module procedure data_res_phainv_2dfft_mask
...
!===================================================================
subroutine csg_filt_1d(csg,nt,dt,f11,f22,f33,f44)
real                :: csg(:),f11,f22,f33,f44
real,   intent(in)  :: dt
integer, intent(in) :: nt
integer             :: ig,kp,nn
real, allocatable   :: trc(:),trc_filt(:),F(:)

call allocate_and_initial(trc,nt)
call allocate_and_initial(trc_filt,nt)
call nextpow2(nt,kp)
nn = 4*(2**kp)
allocate(F(nn))
F=0.0
...
trc = csg    
call bp_filter(trc,trc_filt,F,nt,dt,f11,f22,f33,f44)
csg=trc_filt

end subroutine csg_filt_1d

!===================================================================
subroutine csg_filt_2d(csg,nt,dt,ng,f11,f22,f33,f44)
real                :: csg(:,:),f11,f22,f33,f44
real,   intent(in)  :: dt
...
do ig=1,ng
    ...
    csg(:,ig)=trc_filt
enddo
end subroutine csg_filt_2d

!===================================================================
subroutine csg_conv_sou_2d(dpre,dobs,nt,dt,ng,fpk)
real  :: dpre(:,:),dobs(:,:)
integer, intent(in) :: nt,ng
real, intent(in)    :: dt,fpk

call data_conv(dpre,nt,ng,dt,fpk) 
call data_conv(dobs,nt,ng,dt,fpk) 

end subroutine csg_conv_sou_2d


!=====================================================================
subroutine ricker0_1d(s, nt, f, dt)
...
s=s/maxval(abs(s))
end subroutine ricker0_1d


subroutine data_conv_2d(csg,nt,ng,dt,fpk)
real                :: csg(:,:)
integer,intent(in)  :: nt,ng
real,intent(in)     :: dt,fpk
integer             :: ig
...
do ig=1,ng
  tmp = csg(:,ig)
  call data_conv_1d(tmp,nt,dt,fpk)
  csg(:,ig)=tmp
enddo
end subroutine data_conv_2d

subroutine data_conv_1d(trace,nt,dt,fpk)
integer,intent(in)  :: nt
real, intent(in)    :: dt,fpk
real                :: trace(:)
real,allocatable    :: sou(:),d_cov(:)
integer             :: it,k,kp,nf,npk
complex,allocatable :: C_sou(:),C_trc(:),C_tmp(:),trace_work(:),trcp(:),soup(:)
integer(kind=8)     :: plan, plan1,plan2
complex             :: i
...
call allocate_and_initial(sou,nt)
call allocate_and_initial(d_cov,nt)
...
call ricker0(sou,nt,fpk,dt)
npk = floor(1/fpk/dt)

do it=1,nt
   if (trace(it)<0) trace(it)=0
enddo

do k=1,nf
  if (k<=nt) then
      soup(k) = cmplx(sou(k),0.0)
  endif
  if (k>nt) then
      soup(k)= cmplx(0.0,0.0);
  endif
enddo
do k=1,nf
  if (k<=nt) then
      trcp(k) = cmplx(trace(k),0.0)
  endif
  if (k>nt) then
...
call sfftw_plan_dft_1d(plan,nf,soup,C_sou,FFTW_FORWARD,FFTW_ESTIMATE)
call sfftw_execute(plan,soup,C_sou)
call sfftw_destroy_plan(plan)


call sfftw_plan_dft_1d(plan1,nf,trcp,C_trc,FFTW_FORWARD,FFTW_ESTIMATE)
call sfftw_execute(plan1,trcp,C_trc)
call sfftw_destroy_plan(plan1)

C_tmp=C_sou*C_trc

call sfftw_plan_dft_1d(plan2,nf,C_tmp,trace_work,FFTW_BACKWARD,FFTW_ESTIMATE)
call sfftw_execute(plan2,C_tmp,trace_work)
call sfftw_destroy_plan(plan2)
d_cov = real(trace_work(1+npk:nt+npk))/nf
trace = d_cov

end subroutine data_conv_1d
...
integer,intent(in) :: nt,ng
real, intent(out) :: res_data(:,:),res(:)
real,allocatable  :: trace_pre(:),trace_obs(:),trace_pre_bp(:),trace_obs_bp(:),F(:),dpre_bp(:,:),dobs_bp(:,:)
integer :: it,ig,nn,kp
real :: norm_pre,norm_obs
...
call allocate_and_initial(dpre_bp,nt,ng)
call allocate_and_initial(dobs_bp,nt,ng)
!call norm_shot(dpre,nt,ng)
!call norm_shot(dobs,nt,ng)
...
call csg_filt(dpre,nt,dt,ng,f1,f2,f3,f4)
call csg_filt(dobs,nt,dt,ng,f1,f2,f3,f4)
if (N.eq.-1) then
  !call norm_trace(dpre,nt,ng)
  call norm_shot(dpre,nt,ng)
  !call norm_trace(dobs,nt,ng)
  call norm_shot(dobs,nt,ng)
  do ig=1,ng
   res_adj(:,ig) = dobs(:,ig)-dpre(:,ig)
   res(ig)= sum((res_adj(:,ig))**2) 
  enddo
else
...
! get the F, lowpass filter, ifft to get trace_obs_bar
 call bp_filter(trace_pre,trace_pre_bp,F,nt,dt,f1,f2,f3,f4)
 Ctmp1=F*Mag_trc_pre*exp(i*Pha_trc_obs) !use the predicted spectrum
...
! lowpass filter, ifft to get trace_pre_bar
 Ctmp2=F*Mag_trc_pre*exp(i*Pha_trc_pre)  ! use the predicted spectrum
...
subroutine data_res_phainv_2dfft(dpre,dobs,nt,ng,dt,res_adj,res,N,f1,f2,f3,f4,use_amp)
integer,intent(in) :: nt,ng,use_amp
real, intent(in)   :: dt,f1,f2,f3,f4
real               :: dpre(:,:),dobs(:,:)
real,allocatable   :: dpre_ext(:,:),dobs_ext(:,:)
real,allocatable   :: res_adj(:,:)
real,allocatable   :: trace_pre(:),trace_pre_bar(:),trace_pre_barN(:),trace_obs(:),trace_obs_bar(:),trace_obs_barN(:),dp(:),dpN(:),trace(:),res(:),trace_pre_bp(:)
integer            :: it,k,ig,N,kp,ntf,ngf
complex,allocatable :: C_pre(:,:), C_obs(:,:),C_trc_obs(:),C_trc_pre(:),trcp(:),trace_work(:)
complex,allocatable :: Ctmp1(:,:),Ctmp2(:,:)
integer(kind=8)    :: plan, plan1
real,allocatable   :: tmp_dpre_bar(:,:),tmp_dobs_bar(:,:), dpre_bar(:,:), dobs_bar(:,:),Mag_obs(:,:),Mag_pre(:,:),Mag_new(:,:),Pha_obs(:,:),Pha_pre(:,:),F(:)
complex :: i
...
allocate(dpre_ext(ntf,ngf))
allocate(dobs_ext(ntf,ngf))
dpre_ext=0.0
dobs_ext=0.0
dpre_ext(1:nt,1:ng)=dpre(:,:)
dobs_ext(1:nt,1:ng)=dobs(:,:)

allocate(C_pre(ntf/2+1,ngf))
C_pre=0.0
allocate(C_obs(ntf/2+1,ngf))
C_obs=0.0
allocate(Mag_pre(ntf/2+1,ngf))
Mag_pre=0.0
allocate(Mag_obs(ntf/2+1,ngf))
Mag_obs=0.0
allocate(Mag_new(ntf/2+1,ngf))
Mag_new=0.0
allocate(Pha_pre(ntf/2+1,ngf))
Pha_pre=0.0
allocate(Pha_obs(ntf/2+1,ngf))
Pha_obs=0.0
allocate(Ctmp1(ntf/2+1,ngf))
Ctmp1=0.0
allocate(Ctmp2(ntf/2+1,ngf))
Ctmp2=0.0
allocate(tmp_dpre_bar(ntf,ngf))
tmp_dpre_bar=0.0
allocate(tmp_dobs_bar(ntf,ngf))
tmp_dobs_bar=0.0
allocate(dpre_bar(nt,ng))
dpre_bar=0.0
allocate(dobs_bar(nt,ng))
dobs_bar=0.0


call sfftw_plan_dft_r2c_2d(plan,ntf,ngf,dpre_ext,C_pre,FFTW_ESTIMATE)
call sfftw_execute_dft_r2c(plan, dpre_ext, C_pre)
call sfftw_destroy_plan(plan)

call sfftw_plan_dft_r2c_2d(plan,ntf,ngf,dobs_ext,C_obs,FFTW_ESTIMATE)
call sfftw_execute_dft_r2c(plan, dobs_ext, C_obs)
call sfftw_destroy_plan(plan)

Mag_pre = abs(C_pre)
Pha_pre = atan2(aimag(C_pre),real(C_pre))

Mag_obs = abs(C_obs)
Pha_obs = atan2(aimag(C_obs),real(C_obs))
...
Ctmp1=Mag_new*exp(i*Pha_pre) 
Ctmp2=Mag_new*exp(i*Pha_obs)


call sfftw_plan_dft_c2r_2d(plan1,ntf,ngf,Ctmp1,tmp_dpre_bar,FFTW_ESTIMATE)
call sfftw_execute_dft_c2r(plan1,Ctmp1,tmp_dpre_bar)
call sfftw_destroy_plan(plan1)
dpre_bar = tmp_dpre_bar(1:nt,1:ng)

call sfftw_plan_dft_c2r_2d(plan1,ntf,ngf,Ctmp2,tmp_dobs_bar,FFTW_ESTIMATE)
call sfftw_execute_dft_c2r(plan1,Ctmp2,tmp_dobs_bar)
call sfftw_destroy_plan(plan1)
dobs_bar = tmp_dobs_bar(1:nt,1:ng)

do ig=1,ng

trace_pre_bar = dpre_bar(:,ig)
trace_obs_bar = dobs_bar(:,ig)

dp = dobs_bar(:,ig) - dpre_bar(:,ig);
call d_integerN(dp,nt,dpN,N-1)
res_adj(:,ig) = dpN
...
res(ig)= sum((trace_obs_barN - trace_pre_barN)**2)/ng

enddo

endif

end subroutine data_res_phainv_2dfft

subroutine data_res_phainv_2dfft_mask(dpre,dobs,nt,ng,dt,res_adj,res,N,f1,f2,f3,f4,mask_csg)
integer,intent(in) :: nt,ng
real, intent(in)   :: dt,f1,f2,f3,f4
real               :: dpre(:,:),dobs(:,:),mask_csg(:,:)
real,allocatable   :: dpre_ext(:,:),dobs_ext(:,:)
real,allocatable   :: res_adj(:,:),res_tmp(:,:)
real,allocatable   :: trace_pre(:),trace_pre_bar(:),trace_pre_barN(:),trace_obs(:),trace_obs_bar(:),trace_obs_barN(:),dp(:),dpN(:),trace(:),res(:),trace_pre_bp(:)
integer            :: it,k,ig,N,kp,ntf,ngf
complex,allocatable :: C_pre(:,:), C_obs(:,:),C_trc_obs(:),C_trc_pre(:),trcp(:),trace_work(:)
complex,allocatable :: Ctmp1(:,:),Ctmp2(:,:)
...
call allocate_and_initial(dpN,nt)
call allocate_and_initial(res,ng)
call allocate_and_initial(res_tmp,nt,ng)
...
if (N.eq.-1) then
  ...
  do ig=1,ng
   res_adj(:,ig) = dobs(:,ig)-dpre(:,ig)
  enddo
   res_adj=res_adj*mask_csg
  do ig=1,ng
   res(ig)= sqrt(sum((res_adj(:,ig))**2)/ng) 
  enddo
else
...
do ig=1,ng
        ...
        res_tmp(:,ig)= trace_obs_barN - trace_pre_barN
enddo
        res_adj = res_adj*mask_csg
        res_tmp = res_tmp*mask_csg
do ig=1,ng
        res(ig)= sqrt(sum((res_tmp(:,ig))**2)/ng)
enddo

endif

end subroutine data_res_phainv_2dfft_mask
