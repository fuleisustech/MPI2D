...
subroutine eikfd3d_fir(s0,t0,zs0,ys0,xs0,nz0,ny0,nx0,h0,vmax0,vmin0)
! inclue "eik3d.pa1"
real,intent(in) :: h0,vmax0,vmin0,zs0,ys0,xs0
real,intent(in) :: s0(:,:,:)
!integer,intent(in) :: nz0, ny0, nx0, ncp0
integer,intent(in) :: nz0, ny0, nx0
real,intent(out):: t0(:,:,:)
integer,allocatable :: mxwk1(:),mywk1(:),mzwk1(:)
integer :: ntt, nt, i, j, jj1, i1, j1, k1
real    :: xs, ys, zs, vmax, vmin, ddx, dt, t00
real,allocatable :: temp(:,:,:)
logical :: isPad
isPad=.false.
call copy_variable(zs0,zs,ys0,ys,xs0,xs,h0,h,vmax0,vmax,vmin0,vmin)
!call copy_variable(nz0,nz,ny0,ny,nx0,nx,ncp0,ncp)
call copy_variable(nz0,nz,ny0,ny,nx0,nx)
if (xs<2.0*h .or. ys<2.0*h .or. zs<2.0*h) then
   isPad=.true.
   call allocate_and_initial(temp,nx,ny,nz)
   call permute(s0,temp,3,2,1)
   nx=nx+10
   ny=ny+10
   nz=nz+10
   xs=xs+5.0*h
...
if (isPad) then
   !call Padding(temp,nx-10,5,5,ny-10,5,5,nz-10,5,5,1.0/vmin,s_work)
   call padarray(temp,nx-10,5,5,ny-10,5,5,nz-10,5,5,s_work,"replicate")
...
do i=1,ns
   ntwk=ntwk+1
   call ind2sub(nxs,nys,indx(i),i1,i2,i3)
   mxwk(ntwk)=i1+ix1-1  
   mywk(ntwk)=i2+iy1-1  
   mzwk(ntwk)=i3+iz1-1
enddo
         ...
         do k=k1-2,k1+2
            ...
            if (i==1.or.i==nx.or.j==1.or.j==ny.or.k==1.or.k==nz) then
               ...
               if (xl.lt.1.5) then
                  ...
                  ntwk=ntwk+1; mxwk(ntwk)=i; mywk(ntwk)=j; mzwk(ntwk)=k
               endif 
            endif
         enddo
      enddo
...
12    continue   
   ...
   iq=min(iq,n)
   !if (iq<0) write(*,*)iq
   !if (mx(iq)<0) write(*,*)mx(iq)
   !if (my(iq)<0) write(*,*)my(iq)
   !if (mz(iq)<0) write(*,*)mz(iq)
...
22 if (i.le.n) then
   ...
   goto 20
30 jstack=jstack+2
   if (jstack.gt.nstack) write(*,*)"nstack must be made larger!"
   if (ir-i.ge.i-l) then
      istack(jstack)=ir
      istack(jstack-1)=i+1
      ir=i-1
